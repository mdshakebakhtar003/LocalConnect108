...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      setLoading(true);
      try {
        const url = `${serverUrl}/user/profile/${user?.email}`;
  ...
  const savechanges = async () => {
    ...
    try {
      ...
      await axios.post(`${serverUrl}/user/edit`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      Alert.alert('Success', 'Profile updated successfully');
      // Refresh profile data
      const res = await axios.get(`${serverUrl}/user/profile/${user?.email}`);
      setUserData(res.data.user);
  ...
  const handlePayments = () => {
    router.replace('profileuser/payments');
  };
  const handlePrivacy = () => {
    router.replace('profileuser/privacy');
  };
  const handleHelp = () => {
    router.replace('profileuser/help');
  };
  const handleSettings = () => {
    router.replace('profileuser/setting');
  };
  const handleLogout = () => {
    Alert.alert(
      'Logout',
